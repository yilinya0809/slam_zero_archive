...
class Local_costmap_publisher{
	...
	public:
		...
		Local_costmap_publisher() {
			...
			gear_state_sub = nh.subscribe("/gear_state", 1, &Local_costmap_publisher::v_callback, this);
			...
			v_sub_ = nh.subscribe("/vehicle_state", 2, &Local_costmap_publisher::v_callback, this);
			cost_map_pub = nh.advertise<nav_msgs::OccupancyGrid>("/costmap", 2);
		}
		...
		void callback(const slam::Data data){
			...
			imshow("local_costmap", local_costmap);
			int key = waitKey(30);
...
int main(int argc, char **argv){
	ros::init(argc, argv, "local_costmap_publisher");
        ...
        ros::Rate rate(10); //ROS rate at ??Hz
	ros::NodeHandle nh;
	while(nh.ok()){
		ros::spinOnce();
		rate.sleep();
