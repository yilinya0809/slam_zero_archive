...
class ObstacleDetector{
	...
	pub_bool_ = nh_.advertise<std_msgs::Bool>("obstacle_existence",2);
        ...
        lidar_height_ = 0.87;
        plane_tolerance_ = 0.2;
        cluster_tolerance_ = 0.20;
        cluster_threshold_ = 4;
        mission_state_ = 7;
    ...
    void removing_plane(){
        ...
        //pitch_offset = 0.02;
        plane_config_[0]=0.0; //-sin(lidar_angle_*M_PI/180 + pitch_offset + lidar_angle_shift);
        plane_config_[1]=0.0;
        plane_config_[2]=1.0; // cos(lidar_angle_*M_PI/180 + pitch_offset + lidar_angle_shift);
	...
	if (!cluster_counts.empty()) is_obstacle = true;
	rt_bool.data = is_obstacle;	
        
	pub_.publish(rt);
	pub_count_.publish(rt_count);
	pub_bool_.publish(rt_bool);
        clock_t end = clock();
